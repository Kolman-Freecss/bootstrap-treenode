 ...
 * =========================================================
 * Modifications 2019 Sergio Martínez
...
;(function ($, window, document, undefined) {
	...
	Tree.prototype.subscribeEvents = function () {
		...
		if (typeof (this.options.onInputChanges) === 'function') {
			this.$element.on('inputChanges', this.options.onInputChanges);
		}
	};
	...
	*/
	...
	Tree.prototype.buildTree = function (nodes, level) {
		...
		if (!nodes) return;
		level += 1;
		var paddingLenght = (level - 1) * 3;

		var _this = this;
		$.each(nodes, function addNodes(id, node) {

			var treeItemLi = $(_this.template.item)
			...
			var treeItem;
			// Add bootstrap row
			if (_this.options.enableRow) {
				var row = $(_this.template.row);
				treeItemLi
					.append(row);
				treeItem = row;
			}
			...
			iconLabelDiv
				.append($(_this.template.icon)
					...
					.addClass(_this.options.iconSize ? 'col-sm-' + _this.options.iconSize : '')
				);
			...
			if (_this.options.enableLinks) {
			...
			else if (!_this.options.enableLabels){
				// otherwise just text
				iconLabelDiv
					.append(node.text);
			}
				...
				iconLabelDiv
					.append($(_this.template.label)
						.addClass(_this.options.labelSize ? 'col-sm-' + _this.options.labelSize : '')
							.append(node.text)
			...
			if (_this.options.showInputTexts) {
				...
				treeItem
					.append(input
						...
						.change(function() {inputValueChanged(node, input, _this)})
					);
			}
			...
			_this.$wrapper.append(treeItemLi);
	...
	function inputValueChanged(node, inputField, $scope)
	{
		node.inputValue = inputField.val();
		
		if(node.inputValue)
		{
			if(node.nodes)
				$scope.enableNode(node.nodes);
			
		}
		else {
			if(node.nodes)
				$scope.disableNode(node.nodes);
		}
		
		$scope.$element.trigger('inputChanges', $.extend(true, {}, node));
	}
